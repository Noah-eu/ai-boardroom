...
export function AgentsPanel({ mode = 'desktop' }: AgentsPanelProps) {
  ...
  return (
    <div className={`h-full flex flex-col bg-gray-950 ${isMobile ? '' : 'border-l border-gray-800'}`}>
      {/* Header */}
      <div className={`flex-shrink-0 border-b border-gray-800 ${isMobile ? 'px-6 py-5' : 'px-4 py-3'}`}>
        <h2 className={`${isMobile ? 'text-lg' : 'text-sm'} font-semibold text-gray-100`}>{t('agents.title')}</h2>
        <p className={`${isMobile ? 'mt-2 text-base' : 'mt-0.5 text-xs'} text-gray-400`}>
          {t('agents.phase')}:{' '}
      ...
      {/* Workflow progress */}
      <div className={`flex-shrink-0 border-b border-gray-800/50 ${isMobile ? 'px-6 py-4' : 'px-4 py-2'}`}>
        <div className="flex flex-wrap items-center gap-3">
          {phaseOrder.filter((p) => p !== 'idle' && p !== 'summary').map((phase) => {
            ...
            return (
              <span
                key={phase}
                className={`${isMobile ? 'text-sm px-3 py-2 rounded-xl' : 'text-[9px] px-1.5 py-0.5 rounded'} font-medium ${
                  isCurrent
      ...
      {/* Agent list */}
      <div className={`flex-1 overflow-y-auto space-y-6 ${isMobile ? 'px-5 py-5' : 'px-3 py-3'}`}>
        {/* Debate group */}
        <div>
          <p className={`${isMobile ? 'mb-4 text-sm' : 'mb-2 text-[10px]'} px-1 font-semibold uppercase tracking-widest text-gray-400`}>
